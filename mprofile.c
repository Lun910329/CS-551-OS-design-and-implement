...
int set_menv(char *buff){
    ...
    // syscl: set alarm var
    if (strstr(buff, "setAlarm=1") != NULL)
    {
        puts("setAlarm is enable by profile, set alarm");
        puts("Use init_alarm to turn on or turn off setAlarm");
        kSetAlarm = kTRUE;
    }
    else if (strstr(buff, "setAlarm=0") != 0)
    {
        puts("setAlarm is disable by profile, disable alarm");
        puts("Use init_alarm to turn on or turn off setAlarm");
        kSetAlarm = kFALSE;
    }
    ...
    *p='\0';
    ...
    {
        // syscl: a patch to fix jump issue due to '\n'
        //for (int ii = 0; )
        // we've read home dir, now jump to home(~)
        printf("home(~) is %s\n", envvalue);
        //char ph[]="/root";
        //chdir(envvalue);
